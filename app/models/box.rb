class Box < ApplicationRecord
  ...
  def generate_customer_boxes
    ...
    bimonthly_subscribers = subscribers.bimonthly.where.not(last_box_date: self.date - 2.weeks..self.date).current_sub
    monthly_subscribers = subscribers.monthly.where.not(last_box_date: self.date - 3.weeks..self.date).current_sub
