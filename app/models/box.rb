class Box < ApplicationRecord
  ...
  def generate_customer_boxes
    ...
    bimonthly_subscribers = subscribers.bimonthly.where(last_box_date: self.date < self.date - 2.weeks).current_sub
    monthly_subscribers = subscribers.monthly.where(last_box_date: self.date < self.date - 3.weeks).current_sub
